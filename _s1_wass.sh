#!/bin/bash
# Stage 1: Wasserstein 2500 steps (for C1/C2/C5 curricula)
GPU=1
...
for idx in 18 50 10 6 8 5; do
  echo "=== $(date) Wasserstein CVA[$idx] GPU=$GPU ==="
  $PY train_rl_fwi.py \
    --policy_type gaussian --model_source smooth --smooth_root data/smooth_models_v2 \
    --cva_file_idx $idx --geometry transmission \
    --fwi_type wasserstein --reward_l2_weight 1.0 --reward_l1_weight 0.0 --reward_tt_weight 0.0 --reward_prior_weight 0.0 \
    ...
    --out_dir runs/curr_wass2500_cva${idx} --device cuda \
    --early_stop_patience 500 --early_stop_window 100 --save_every 100 --seed 42
done
echo "GPU1 ALL DONE (Wass 2500)"
