...
for idx in 18 50 10 6 8 5; do
  CKPT=runs/curr_tt2500_cva${idx}/policy_best.pt
  if [ ! -f "$CKPT" ]; then echo "MISSING: $CKPT"; continue; fi
  echo "=== $(date) C3: TT→Contrastive CVA[$idx] GPU=$GPU ==="
  $PY train_rl_fwi.py \
    ...
    --out_dir runs/curr_C3_TT_Contra_cva${idx} --device cuda \
    --resume_ckpt $CKPT \
