#!/bin/bash
# C4: NCC_maxlag 2500 → Contrastive 2500
GPU=1
...
for idx in 18 50 10 6 8 5; do
  CKPT=runs/curr_nccm2500_cva${idx}/policy_best.pt
  if [ ! -f "$CKPT" ]; then echo "MISSING: $CKPT"; continue; fi
  echo "=== $(date) C4: NCCm→Contrastive CVA[$idx] GPU=$GPU ==="
  $PY train_rl_fwi.py \
    --policy_type gaussian --model_source smooth --smooth_root data/smooth_models_v2 \
    --cva_file_idx $idx --geometry transmission \
    --fwi_type contrastive --reward_l2_weight 1.0 --reward_l1_weight 0.0 --reward_tt_weight 0.0 --reward_prior_weight 0.0 \
    ...
    --out_dir runs/curr_C4_NCCm_Contra_cva${idx} --device cuda \
    --resume_ckpt $CKPT \
