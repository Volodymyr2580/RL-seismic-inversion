#!/bin/bash
# C5: Wasserstein 2500 → NCC_maxlag 2500
GPU=2
...
for idx in 18 50 10 6 8 5; do
  CKPT=runs/curr_wass2500_cva${idx}/policy_best.pt
  if [ ! -f "$CKPT" ]; then echo "MISSING: $CKPT"; continue; fi
  echo "=== $(date) C5: Wass→NCC_maxlag CVA[$idx] GPU=$GPU ==="
  $PY train_rl_fwi.py \
    --policy_type gaussian --model_source smooth --smooth_root data/smooth_models_v2 \
    --cva_file_idx $idx --geometry transmission \
    --fwi_type ncc_maxlag --reward_l2_weight 1.0 --reward_l1_weight 0.0 --reward_tt_weight 0.0 --reward_prior_weight 0.0 \
    ...
    --out_dir runs/curr_C5_Wass_NCCm_cva${idx} --device cuda \
    --resume_ckpt $CKPT \
